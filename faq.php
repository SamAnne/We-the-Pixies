...
<?php
					if(isset($_SESSION["editFaq"])){
						        if($usertype == 1  && $_SESSION["editFaq"] != 1){
?>
							<form method="post">
...
<?php
					...
					if(!isset($_SESSION["editFaq"]) && $usertype == 1){ ?>
						<form method="post">
                                                                 <input class="edit-icon" type="submit" value="" title="Edit"  name="editButton">
  <?php
                                                                  if(isset($_POST["editButton"])){
...
<?php 
	$row = mysqli_query($dbc, "SELECT * FROM wtpposts WHERE postType='faq'");
	$result = mysqli_fetch_assoc($row);
	$about = $result["content"];
?>
					<div class="edit-form">
<?php
					if(isset($_SESSION["editFaq"]) && $_SESSION["editFaq"] == 1 && $usertype == 1){?>
							<form method="post">
								<textarea name="editFaq" id="editAbout" style="margin-top:30px;" rows="20" cols="90"><?php echo $about?></textarea>
								<br>
								<br>
								<input type="submit" class="submit" value="Save" name="save">
<?php
						if(isset($_POST["save"])){
								  $newAbout = mysqli_real_escape_string($dbc, $_POST["editFaq"]);
								  mysqli_query($dbc, "UPDATE wtpposts SET content='$newAbout' WHERE postType='faq'");
								$_SESSION["editFaq"] = 0;
